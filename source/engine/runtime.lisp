...
(defgeneric load-compiled-composite (backend compiled-code composite header-object compile-lazy)
...
(defgeneric compile-source-code (backend source-code)
  (:documentation "Compiles the stacked source-code[str]."))
...
(defmacro with-lazy-compilation-mode (&body body)
  `(let ((*lazy-cache-buffer* "")
	 (*lazy-compile-mode* t))
     (progn ,@body)
     (compile-source-code (runtimeconfig-name *runtime*) *lazy-cache-buffer*)))

(defun realize (uop-graph composite &key (function-name nil) (compile-lazy *lazy-compile-mode*))
...
- function-name[string] If set to nil, the function name is autogenerated based on the composite name, shapes, and gensym to cause no conflicts."
  ...
  (flet ((->make-const (scalar)
	...
	(when compile-lazy
	  (setf *lazy-cache-buffer* (format nil "~a~%~a" *lazy-cache-buffer* compiled-code)))
	(let ((compiled-composite
		(load-compiled-composite (runtimeconfig-name *runtime*) compiled-code composite header compile-lazy)))
